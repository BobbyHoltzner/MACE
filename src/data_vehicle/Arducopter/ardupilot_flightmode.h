...
{
public:
    std::string Descriptor = "FLIGHTMODE";

    enum SUBMESSAGE{
        HEARTBEATUPDATE
    };

    struct STRUCT_HEARTBEATUPDATE{
        int vehicleType;
        int vehicleMode;
    };
...
public:
    ...
    void receivedMessage(const int &msgType, std::shared_ptr<TYPE> &data);
    ...
    void getVehicleMode(std::string vehicleMode);
    void getVehicleMode(int vehicleMode);
