...
using CONTROLLER_BOUNDARY_TYPE = GenericController<
    MESSAGETYPE,
    TransmitQueueWithKeys<MessageModuleTransmissionQueue<MESSAGETYPE>, ObjectIntTuple<MaceCore::ModuleCharacteristic>, ObjectIntTuple<BoundaryItem::BoundaryKey>>,
    uint8_t,
    DataItem<BoundaryItem::BoundaryKey, BoundaryList>
>;

template <typename MESSAGETYPE>
using SendBoundaryHelper_RequestDownload = ActionSend<
    MESSAGETYPE,
    CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
...
template <typename MESSAGETYPE>
using SendBoundaryHelper_RequestList = ActionIntermediate<
    MESSAGETYPE,
    CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
    BoundaryItem::BoundaryKey,
    BoundaryItem::BoundaryKey,
...
template <typename MESSAGETYPE>
using SendBoundaryHelper_ReceiveCountRespondItemRequest = ActionIntermediate<
    MESSAGETYPE,
    CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
    BoundaryItem::BoundaryKey,
    BoundaryItem::BoundaryKey,
...
template <typename MESSAGETYPE>
using SendBoundaryHelper_ReceiveCountRespondItemRequest_FromRequest = ActionIntermediate<
    MESSAGETYPE,
    CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
...
template <typename MESSAGETYPE>
using SendBoundaryHelper_RequestItem = ActionIntermediate<
    MESSAGETYPE,
    CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
    BoundaryItem::BoundaryKey,
    BoundaryItem::BoundaryKey,
...
template <typename MESSAGETYPE>
using SendBoundaryHelper_ReceiveItem = ActionIntermediateReceive<
    MESSAGETYPE,
    CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
    BoundaryItem::BoundaryKey,
    BoundaryItem::BoundaryKey,
...
template <typename MESSAGETYPE>
using SendBoundaryHelper_Final = ActionFinalReceiveRespond<
    MESSAGETYPE,
    CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
    BoundaryItem::BoundaryKey,
...
template <typename MESSAGETYPE>
using SendBoundaryHelper_FinalFinal = ActionFinish<
    MESSAGETYPE,
    CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
    BoundaryItem::BoundaryKey,
    uint8_t,
...
template <typename MESSAGETYPE>
class ControllerBoundary : public CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>,
        public SendBoundaryHelper_RequestDownload<MESSAGETYPE>,
        public SendBoundaryHelper_RequestList<MESSAGETYPE>,
        public SendBoundaryHelper_ReceiveCountRespondItemRequest<MESSAGETYPE>,
        public SendBoundaryHelper_ReceiveCountRespondItemRequest_FromRequest<MESSAGETYPE>,
        public SendBoundaryHelper_RequestItem<MESSAGETYPE>,
        public SendBoundaryHelper_ReceiveItem<MESSAGETYPE>,
        public SendBoundaryHelper_Final<MESSAGETYPE>,
        public SendBoundaryHelper_FinalFinal<MESSAGETYPE>
//        public Action_RequestCurrentBoundary_Initiate<MESSAGETYPE>,
//        public Action_RequestCurrentBoundary_Response<MESSAGETYPE>,
//        public Action_RequestCurrentBoundary_NoBoundaryResponse<MESSAGETYPE>
{
...
protected:
    ...
    virtual bool BuildData_Send(const mace_boundary_request_list_t &cmd, const MaceCore::ModuleCharacteristic &sender, mace_boundary_count_t &rtn, MaceCore::ModuleCharacteristic &vehicleObj, BoundaryItem::BoundaryKey &receiveQueueObj, BoundaryItem::BoundaryKey &respondQueueObj)
    {
        ...
        CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::FetchDataFromKey(key, boundaries);
    ...
    virtual bool BuildData_Send(const mace_boundary_request_item_t &boundaryRequest, const MaceCore::ModuleCharacteristic &sender, mace_boundary_item_t &boundaryItem, MaceCore::ModuleCharacteristic &vehicleObj, BoundaryItem::BoundaryKey &receiveQueueObj, BoundaryItem::BoundaryKey &respondQueueObj)
    {
        ...
        if(m_BoundariesUploading.find(key) == m_BoundariesUploading.cend())
        {
            if(CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog)
                CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog->error("BoundaryController_ExternalLink has been told to transmit a boundary item from a boundary which keys dont match the contained.");
            return false;
        }
        ...
        if(index >= m_BoundariesUploading[key].getQueueSize())
        {
            ...
            if(CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog)
                CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog->error("BoundaryController_ExternalLink has been told to transmit a boundary item with index " + std::to_string(index) + " which is greater than the size of the list contained.");
            return false;
        }

        if(CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog)
            CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog->info("BoundaryController_ExternalLink has been told to transmit a boundary item with index " + std::to_string(index) + ".");
    ...
    virtual bool BuildData_Send(const mace_boundary_item_t &boundaryItem, const MaceCore::ModuleCharacteristic &sender, mace_boundary_request_item_t &request, MaceCore::ModuleCharacteristic &vehicleObj, BoundaryItem::BoundaryKey &receiveQueueObj, BoundaryItem::BoundaryKey &respondQueueObj)
    {
        ...
        {
            if(CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog)
                CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog->error("Boundary controller received a boundary item with a key that is not equal to the one we were originally told.");
            return false;
        }
        ...
        if(seqReceived > (m_BoundariesBeingFetching[key].boundary.getQueueSize() - 1)) //this should never happen
        {
            ...
            if(CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog)
                CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog->error("Boundary controller received a boundary item with an index greater than available in the queue.");
            return false;
        }
    ...
    virtual bool Construct_FinalObjectAndResponse(const mace_boundary_item_t &boundaryItem, const MaceCore::ModuleCharacteristic &sender, mace_boundary_ack_t &ackBoundary, std::shared_ptr<BoundaryItem::BoundaryList> &finalList, MaceCore::ModuleCharacteristic &vehicleObj, BoundaryItem::BoundaryKey &queueObj)
    {
        ...
        if(CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog)
        {
            ...
            buffer << key;
            CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>::mLog->info("Boundary Controller has received the entire boundary of " + std::to_string(m_BoundariesBeingFetching[key].boundary.getQueueSize()) + " for mission " + buffer.str() + ".");
        }
...
public:
    ...
    ControllerBoundary(const IMessageNotifier<MESSAGETYPE> *cb, MessageModuleTransmissionQueue<MESSAGETYPE> *queue, int linkChan) :
        CONTROLLER_BOUNDARY_TYPE<MESSAGETYPE>(cb, queue, linkChan),
        SendBoundaryHelper_RequestDownload<MESSAGETYPE>(this, mace_msg_boundary_request_list_encode_chan),
        SendBoundaryHelper_RequestList<MESSAGETYPE>(this, mace_msg_boundary_request_list_decode, mace_msg_boundary_count_encode_chan),
        SendBoundaryHelper_ReceiveCountRespondItemRequest<MESSAGETYPE>(this, mace_msg_boundary_count_decode, mace_msg_boundary_request_item_encode_chan),
        SendBoundaryHelper_ReceiveCountRespondItemRequest_FromRequest<MESSAGETYPE>(this, mace_msg_boundary_count_decode, mace_msg_boundary_request_item_encode_chan),
        SendBoundaryHelper_RequestItem<MESSAGETYPE>(this, mace_msg_boundary_request_item_decode, mace_msg_boundary_item_encode_chan),
        SendBoundaryHelper_ReceiveItem<MESSAGETYPE>(this,
                                                    [this](const mace_boundary_request_item_t &A, const MaceCore::ModuleCharacteristic &B, const BoundaryItem::BoundaryKey &C, const MaceCore::ModuleCharacteristic &D){SendBoundaryHelper_ReceiveCountRespondItemRequest<MESSAGETYPE>::NextTransmission(A,B,C,D);},
    mace_msg_boundary_item_decode),
        SendBoundaryHelper_Final<MESSAGETYPE>(this, mace_msg_boundary_item_decode, mace_msg_boundary_ack_encode_chan),
        SendBoundaryHelper_FinalFinal<MESSAGETYPE>(this, mace_msg_boundary_ack_decode)
//        Action_RequestCurrentBoundary_Initiate<MESSAGETYPE>(this, mace_msg_boundary_request_list_encode_chan),
//        Action_RequestCurrentBoundary_Response<MESSAGETYPE>(this,
//                                [this](const mace_boundary_count_t &A, const MaceCore::ModuleCharacteristic &B, const BoundaryItem::BoundaryKey &C, const MaceCore::ModuleCharacteristic &D){SendHelper_RequestList<MESSAGETYPE>::NextTransmission(A,B,C,D);},
//                                mace_msg_boundary_request_list_decode),
//        Action_RequestCurrentBoundary_NoBoundaryResponse<MESSAGETYPE>(this,
//                                [this](const mace_boundary_ack_t &A, const MaceCore::ModuleCharacteristic &B, const BoundaryItem::BoundaryKey &C, const MaceCore::ModuleCharacteristic &D){SendHelper_Final<MESSAGETYPE>::FinalResponse(A,B,C,D);},
//                                mace_msg_boundary_request_list_decode)
    {
    ...
    {
        SendBoundaryHelper_RequestDownload<MESSAGETYPE>::Send(key, sender, target);
    }

    void RequestCurrentBoundary(const MaceCore::ModuleCharacteristic &sender, const MaceCore::ModuleCharacteristic &target)
    {
        //Action_RequestCurrentBoundary_Initiate<MESSAGETYPE>::Request(sender, target);
    }
