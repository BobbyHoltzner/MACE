...
class ActionSend_Broadcast :
        public ActionBase<CONTROLLER_TYPE, MSG_TYPE>
{

    typedef ActionBase<CONTROLLER_TYPE, MSG_TYPE> BASE;
protected:
    virtual void Construct_Send(const DATA_TYPE &, const MaceCore::ModuleCharacteristic &sender, MSG_TYPE &) = 0;

public:


    ActionSend_Broadcast(CONTROLLER_TYPE *controller,
    ...
    {
        MSG_TYPE cmd;
        Construct_Send(commandItem, sender, cmd);
...
class ActionSend_TargetedWithResponse :
        public ActionBase<CONTROLLER_TYPE, MSG_TYPE>
{
...
public:
    ...
    ActionSend_TargetedWithResponse()
    {
        throw std::runtime_error("Default Constructor not supported");
    }

    ActionSend_TargetedWithResponse(CONTROLLER_TYPE *controller,
               const std::function<void(uint8_t system_id, uint8_t, uint8_t, mace_message_t*, const MSG_TYPE*)> &encode_chan) :
        ActionBase<CONTROLLER_TYPE, MSG_TYPE>(controller, encode_chan, {})
    {
