...
PolygonMapIterator PolygonMapIterator::begin() const
{
    PolygonMapIterator newIT(this);
     newIT.it->currentIndex =  newIT.it->startIndex;
    return newIT;
}
...
{
    PolygonMapIterator newIT(this);
    newIT.it->currentIndex =  newIT.it->endIndex;
    return newIT;
}
...
{
    double x,y;
    it->parentMap->getPositionFromIndex(it->currentIndex,x,y);
...
void PolygonMapIterator::boundSubmap(const geometry::Polygon_2DC &boundary)
{
    ...
    this->it->startIndex = this->it->parentMap->indexFromPos(bottomLeft.getXPosition(), bottomLeft.getYPosition());
    this->it->endIndex  = this->it->parentMap->indexFromPos(upperRight.getXPosition(), upperRight.getYPosition());
}

void PolygonMapIterator::findValidStartIndex()
{
    this->it->currentIndex = this->it->startIndex;
