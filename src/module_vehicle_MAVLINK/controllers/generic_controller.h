...
#include "controllers/base_data_item.h"
...
public:
    ...
    void AddResponseLogic(DECODE_FUNC func, const std::function<KEY(const DECODE_TYPE&, const MaceCore::ModuleCharacteristic &sender)> &keyExtractor, const std::function<void(const DECODE_TYPE &msg, KEY &key, const MaceCore::ModuleCharacteristic &sender)> &action)
    {
        ...
        auto newItem = std::make_tuple(MavlinkMessageIDEq<I>(),
                                       MavlinkProcessFSMState<DECODE_TYPE>(func, [this, action, keyExtractor](const DECODE_TYPE &msg, const MaceCore::ModuleCharacteristic &sender)
                                        {
    ...
    void AddTriggeredLogic(DECODE_FUNC func, const std::function<void(const DECODE_TYPE &msg, const MaceCore::ModuleCharacteristic &sender)> &action)
    {
        ...
        auto newItem = std::make_tuple(MavlinkMessageIDEq<I>(),
                                       MavlinkProcessFSMState<DECODE_TYPE>(func, [this, action](const DECODE_TYPE &msg, const MaceCore::ModuleCharacteristic &sender)
                                        {
                                            action(msg, sender);
                                        }));
...
private:
    ...
    static std::function<bool(MaceCore::ModuleCharacteristic, const mavlink_message_t*)> MavlinkMessageIDEq()
    {
    ...
    static std::function<void(MaceCore::ModuleCharacteristic, const mavlink_message_t*)> MavlinkProcessFSMState(DECODE_FUNC decode_func, const std::function<void(const DECODE_TYPE &msg, const MaceCore::ModuleCharacteristic &sender)> &func)
    {
        return [decode_func, func](MaceCore::ModuleCharacteristic sender, const mavlink_message_t* message)
        {
