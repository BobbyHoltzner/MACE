    ...
    render() {
        const globalOriginMarker = {
            position: new L.LatLng(
                this.props.globalOrigin.lat,
                this.props.globalOrigin.lng
            ),
            icon: new L.Icon({
        ...
        for (let i = 0; i < this.props.drawPolygonPts.length; i++) {
            drawingMarkers.push(
                <Marker
                    key={i}
                    position={this.props.drawPolygonPts[i]}
                    title={i.toString()}
                    icon={icon}
                    draggable={false}
                />
        ...
        if (this.props.drawPolygonPts.length > 2) {
            ...
            for (let i = 0; i < this.props.gridPts.inPoly.length; i++) {
                tmpGridPts.push(
                    <Marker
                        key={i}
                        position={this.props.gridPts.inPoly[i]}
                        title={i.toString()}
                        icon={gridIcon}
                        draggable={false}
                    />
        ...
        if (this.props.drawPolygonPts.length > 2) {
            ...
            for (let i = 0; i < this.props.gridPts.trimmedPts.length; i++) {
                tmpTrimmedGridPts.push(
                    <Marker
                        key={i}
                        position={this.props.gridPts.trimmedPts[i]}
                        title={i.toString()}
                        icon={trimmedIcon}
                        draggable={false}
                    />
            ...
            <MuiThemeProvider muiTheme={lightMuiTheme}>
                <div style={styles.parentStyle}>
                    ...
                    >
                        ...
                        <LayerGroup>
                            {/* Aircraft Icons */}
                            {Object.keys(this.props.connectedVehicles).map(
                                (key: string) => {
                                    return (
                                        <Marker
                                            zIndexOffset={1000}
                            ...
                            {/* Global Origin */}
                            <Marker
                                position={globalOriginMarker.position}
                                icon={globalOriginMarker.icon}
                            />
                            ...
                            {Object.keys(this.props.connectedVehicles).map(
                                (key: string) => {
                                    ...
                                    for (
                                        let i = 0;
                                        i <
                                        this.props.connectedVehicles[key]
                                            .vehicleMission.latLons.length;
                                        i++
                                    ) {
                                        markers.push(
                                            <Marker
                            ...
                            {/* EnvironmentBoundary */}
                            <Polygon
                                positions={this.props.environmentBoundary}
                                color={colors.white}
