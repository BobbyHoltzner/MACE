...
export class TakeoffDialog extends React.Component<Props, State> {
    constructor(props: Props) {
        ...
        this.state = {
            ...
            takeoffLat: this.props.useContext
                ? this.props.contextAnchor.latlng.lat.toString()
                : "0",
            takeoffLon: this.props.useContext
                ? this.props.contextAnchor.latlng.lng.toString()
                : "0"
        }
    }
    ...
    handleTakeoff = () => {
        if (this.props.useContext) {
            if (this.state.selectedVehicleID !== "0") {
                this.props.handleTakeoff(
                    this.state.selectedVehicleID,
                    this.state.takeoffAlt,
                    this.state.takeoffLat,
                    this.state.takeoffLon
                )
            } else {
                let title = "Takeoff"
                let level = "info"
                this.props.showNotification(
                    title,
                    "Select a vehicle to send takeoff coordinates to.",
                    level,
                    "tc",
                    "Got it"
                )
