...
export class VehicleDB {
    ...
    parseJSONData = (jsonData: TCPReturnType) => {
        ...
        if (jsonData.dataType === "ConnectedVehicles") {
            ...
            console.log(
                "Connected vehicles return: " + jsonVehicles.connectedVehicles
            )
            ...
            for (let i = 0; i < jsonVehicles.connectedVehicles.length; i++) {
                if (
                    stateCopy[jsonVehicles.connectedVehicles[i].toString()] !==
                    undefined
                ) {
                    ...
                    console.log(
                        "Vehicle NOT found: " +
                            jsonVehicles.connectedVehicles[i]
                    )
                    ...
                    newVehicle.highlightColor =
                        "rgba(" + rgb.r + "," + rgb.g + "," + rgb.b + ",1)"
                    newVehicle.opaqueHighlightColor =
                        "rgba(" + rgb.r + "," + rgb.g + "," + rgb.b + ",.2)"
                    stateCopy[
                        jsonVehicles.connectedVehicles[i].toString()
                    ] = newVehicle
                }
            }
            ...
            for (let i = 0; i < idArrays.length; i++) {
                if (
                    jsonVehicles.connectedVehicles.indexOf(
                        parseInt(idArrays[i])
                    ) >= 0
                ) {
                    continue
        ...
        // Vehicle specific data:
            ...
            if (stateCopy[jsonData.vehicleID]) {
                if (jsonData.dataType === "VehiclePosition") {
                    ...
                    stateCopy[vehiclePosition.vehicleID].position.lat =
                        vehiclePosition.lat
                    stateCopy[vehiclePosition.vehicleID].position.lng =
                        vehiclePosition.lng
                    stateCopy[vehiclePosition.vehicleID].position.alt =
                        vehiclePosition.alt
                    stateCopy[vehiclePosition.vehicleID].numSats =
                        vehiclePosition.numSats
                    stateCopy[vehiclePosition.vehicleID].positionFix =
                        vehiclePosition.positionFix

                    stateCopy[
                        vehiclePosition.vehicleID
                    ].updateVehicleMarkerPosition(vehiclePosition)

                    if (
                        stateCopy[vehiclePosition.vehicleID].isNew &&
                        (stateCopy[vehiclePosition.vehicleID].gps.gpsFix !==
                            "NO GPS" ||
                            stateCopy[vehiclePosition.vehicleID].gps.gpsFix !==
                                "GPS NO FIX") &&
                    ...
                    stateCopy[vehicleAttitude.vehicleID].attitude.roll =
                        vehicleAttitude.roll
                    stateCopy[vehicleAttitude.vehicleID].attitude.pitch =
                        vehicleAttitude.pitch
                    stateCopy[vehicleAttitude.vehicleID].attitude.yaw =
                        vehicleAttitude.yaw

                    stateCopy[vehicleAttitude.vehicleID].updateMarkerAttitude(
                        vehicleAttitude
                    )
                    ...
                    stateCopy[vehicleAirspeed.vehicleID].airspeed =
                        vehicleAirspeed.airspeed
                    this.vehicles = stateCopy
                } else if (jsonData.dataType === "VehicleMission") {
                    let vehicleMission = jsonData as TCPMissionType
                    let stateCopy = deepcopy(this.vehicles)
                    stateCopy[vehicleMission.vehicleID].setVehicleMission(
                        vehicleMission
                    )
                    this.vehicles = stateCopy
                } else if (jsonData.dataType === "VehicleHome") {
                    let vehicleHome = jsonData as TCPReturnType &
                        MissionItemType
                    ...
                    stateCopy[vehicleFuel.vehicleID].fuel.batteryRemaining =
                        vehicleFuel.batteryRemaining
                    stateCopy[vehicleFuel.vehicleID].fuel.batteryCurrent =
                        vehicleFuel.batteryCurrent
                    stateCopy[vehicleFuel.vehicleID].fuel.batteryVoltage =
                        vehicleFuel.batteryVoltage

                    this.vehicles = stateCopy
                } else if (jsonData.dataType === "VehicleMode") {
                    let vehicleMode = jsonData as TCPModeType
                    stateCopy[vehicleMode.vehicleID].vehicleMode =
                    ...
                    if (showMessage) {
                        ...
                        stateCopy[vehicleText.vehicleID].messages.unshift({
                            severity: vehicleText.severity,
                    ...
                    stateCopy[jsonFootprint.vehicleID].sensorFootprint =
                        jsonFootprint.sensorFootprint
                    this.vehicles = stateCopy
                } else if (jsonData.dataType === "VehicleGPS") {
                    let jsonGPS = jsonData as TCPGPSType
                    stateCopy[jsonGPS.vehicleID].gps.visibleSats =
                        jsonGPS.visibleSats
                    ...
                    stateCopy[
                        jsonMissionItem.vehicleID
                    ].updateCurrentMissionItem(
                        jsonMissionItem.missionItemIndex,
                        false
                    )
                    this.vehicles = stateCopy
                    ...
                    if (
                        jsonMissionItem.itemIndex ===
                        stateCopy[jsonMissionItem.vehicleID].vehicleMission
                            .icons.length -
                            1
                    ) {
                        stateCopy[
                    ...
                    stateCopy[jsonHeartbeat.vehicleID].general.autopilot =
                        jsonHeartbeat.autopilot
                    stateCopy[jsonHeartbeat.vehicleID].general.commsProtocol =
                        jsonHeartbeat.commsProtocol
                    stateCopy[jsonHeartbeat.vehicleID].general.aircraftType =
                        jsonHeartbeat.aircraftType
                    stateCopy[jsonHeartbeat.vehicleID].general.companion =
                        jsonHeartbeat.companion
                    stateCopy[
                        jsonHeartbeat.vehicleID
                    ].general.lastHeard = new Date()
                    stateCopy[
                        jsonHeartbeat.vehicleID
                    ...
                    stateCopy[
                        jsonVehicleTarget.vehicleID
                    ].currentTarget.active = true
                    stateCopy[
                        jsonVehicleTarget.vehicleID
                    ].currentTarget.distanceToTarget =
                        jsonVehicleTarget.distanceToTarget
                    stateCopy[
                        jsonVehicleTarget.vehicleID
                    ].currentTarget.targetPosition.lat =
                        jsonVehicleTarget.lat
                    stateCopy[
                        jsonVehicleTarget.vehicleID
                    ].currentTarget.targetPosition.lng =
                        jsonVehicleTarget.lng
                    stateCopy[
                        jsonVehicleTarget.vehicleID
                    ].currentTarget.targetPosition.alt =
                        jsonVehicleTarget.alt
                    this.vehicles = stateCopy
                }
            }
        }
