...
export class Vehicle {
    ...
    constructor(
        ...
        let currentTargetHTML =
            "<img src=\"./images/guided-icon.png\" alt=\"Guided icon\" style=\"width:41px; height:41px;\">"
        this.currentTarget = {
    ...
    setVehicleMission(mission: TCPMissionType) {
        ...
        for (let i = 0; i < mission.missionItems.length; i++) {
            this.vehicleMission.descriptions.push(
                mission.missionItems[i].description
            )
            this.vehicleMission.itemTypes.push(mission.missionItems[i].type)
            let tmpLatLng = new L.LatLng(
                mission.missionItems[i].lat
                    ? mission.missionItems[i].lat
                    : prevLatLng.lat,
                mission.missionItems[i].lng
                    ? mission.missionItems[i].lng
                    : prevLatLng.lng
            )
            this.vehicleMission.latLons.push(tmpLatLng)

            let tmpIcon = this.getMarkerIcon(
                mission.missionItems[i].type,
                false
            )
    ...
    updateHomePosition(newHome?: { lat: number; lon: number; alt: number }) {
        ...
        let iconBackgroundColor = this.isSelected
            ? this.highlightColor
            : this.opaqueHighlightColor
        let iconHTML =
            "<div style=\"background-color: " +
    ...
    updateCurrentMissionItem(currentMissionItem: number, clearActive: boolean) {
        ...
        let prevIcon = this.getMarkerIcon(
            this.vehicleMission.itemTypes[previousItem],
            false
        )
        ...
        if (clearActive) {
            currentIcon = this.getMarkerIcon(
                this.vehicleMission.itemTypes[currentMissionItem],
                false
            )
        } else {
            currentIcon = this.getMarkerIcon(
