...
export class AppHelper {
    ...
    parseJSONConfig = (filename: string, restartServer: boolean = true) => {
        ...
        if (jsonConfig.MACEComms) {
            if (jsonConfig.MACEComms.ipAddress) {
                MACEconfig.config.MACEComms.ipAddress =
                    jsonConfig.MACEComms.ipAddress
                if (restartServer) {
            ...
            if (jsonConfig.MACEComms.listenPortNumber) {
                MACEconfig.config.MACEComms.listenPortNumber =
                    jsonConfig.MACEComms.listenPortNumber
                if (restartServer) {
                    this.maceCommsHelper.setupTCPServer()
                }
            }
            if (jsonConfig.MACEComms.sendPortNumber) {
                MACEconfig.config.MACEComms.sendPortNumber =
                    jsonConfig.MACEComms.sendPortNumber
        ...
        if (jsonConfig.GUIInit) {
            if (jsonConfig.GUIInit.mapCenter) {
                let center = {
                    lat: jsonConfig.GUIInit.mapCenter.lat,
                    lng: jsonConfig.GUIInit.mapCenter.lng,
                    alt: 0
                }
        ...
        if (jsonConfig.VehicleSettings) {
            if (jsonConfig.VehicleSettings.defaultTakeoffAlt) {
                MACEconfig.config.VehicleSettings.defaultTakeoffAlt =
                    jsonConfig.VehicleSettings.defaultTakeoffAlt
            }
        }
    ...
    handleSaveMACEConfig = (
        config: ConfigSettingsType,
    ...
    updateMapCenter = (e: L.DragEndEvent) => {
        ...
        MACEconfig.config.GUIInit.mapCenter = {
    ...
    handleSaveTakeoff = (takeoffAlt: string) => {
        let MACEconfig = this.MACEconfig
        MACEconfig.config.VehicleSettings.defaultTakeoffAlt = parseFloat(
            takeoffAlt
        )
        this.MACEconfig = MACEconfig
    }
