...
def main(passed_in_arguments):
    ...
    # Read in arguments:
    argboundaryVertices = np.asarray(json.loads(passed_in_arguments[1]))
    arghotSpotsIn = np.asarray(json.loads(passed_in_arguments[2]))
    arghotSpots = sortWesternPoints(arghotSpotsIn)
    argPVal = json.loads(passed_in_arguments[3])
    argGridVal = json.loads(passed_in_arguments[4])
    argPathDirection = json.loads(passed_in_arguments[5])
    ...
    print len(vehiclePathsLatLon)
    
    # for path in vehimclePathsNormalized:r-')
    ...
    shortenedWPs = []
    tmplist = vehiclePathsLatLon[0]
    for i in xrange(0, 20):
        shortenedWPs.append(tmplist[i])
    # print "The length of the shortened list is: %s" %len(shortenedWPs)
    ...
    aircraftThread = Process(target=newObject.runCommand)
    aircraftThread.daemon = False
