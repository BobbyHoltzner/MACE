...
import json
from trim_waypoints import TrimWaypoints
...
def voronoi(hotSpots, bounding_box, eps=1e-6):
...
def sortWesternPoints(locationArray):
    ...
    arrayOrderSorted = []
    for i in range(len(newlonLocation)):
        for j in range(len(locationArray)):
    ...
    for i in range(len(arrayOrderSorted)):
...
def gen_waypoints(passed_in_arguments):
    ...
    for i in range(len(hotSpotsNormalized)):
        if validatePointsPath.contains_point(hotSpotsNormalized[i]) is False:
            raise AttributeError("One of the selected points of interest is either on or not contained in the boundary: " + str(hotSpotsNormalized[i]))
